...
public class Main {
    ...
    public static void main(String[] args) {
		...
		//This creates 3 sets of groups, each set has 0 to 3 groups of customers, then creates 1 to 10 customers per group.
		//Sets of groups are distributed by normal distribution.  
		double groupsArrivalTime = 0; 
		double temp = 0; 
		for(int k = 0; k < 3; k++) {
			temp = getNormal(50,10);
			groupsArrivalTime += temp; //Calculates the arrival time of the current group
			for(int i = 0; i < (Math.random() * (3 - 0)*10); i+= 10) {  //Generate 0 to 3 groups show up simutaniously, groups have higher time value
				for(int j = 1; j <= (Math.random() * (10 - 1)); j++) { 	//Generate 1 to 10 customers per group
					futureTaskList.push(createFries(i, groupsArrivalTime+i+j)); //Arrival Time is when group arrived, the order of their group of groups plus their position in their group. 
				}
			}
		}
